...
function [Lifespan, PressDrop, TempArray, Qtot] = SKRegen2_Parameterized(Data, NumChannels, WallThickness, AspectRatio, ChannelWidth, DisplayMode, FitCoef, ThrottleIn, OFIn, MdotCoolIn)
New_CEA = false;
fclose all;
close all;
u = convertUnits;
...
if nargin >= 8 && ~isempty(ThrottleIn); throttle = ThrottleIn; else; throttle = 0.5; end
...
if nargin >= 9 && ~isempty(OFIn); total_OF = OFIn; else; total_OF = 1.2; end
total_mdot = (throttle * Ft / Isp_eff) / 2.205; % Total chamber mass flow [kg/s]  
if nargin >= 10 && ~isempty(MdotCoolIn); mdot_coolant = MdotCoolIn; else; mdot_coolant = total_mdot / (1 + total_OF); end
...
while ~global_converged && global_counter < 500
    ...
    for i = 1:steps
        ...
        Nu_turb = ((f / 8) * (max(Re_coolant(i), 1001) - 1000) * Pr_coolant(i)) / ...
                  (1 + 12.7 * ((f / 8) ^ 0.5) * (Pr_coolant(i) ^ (2/3) - 1));
        Nu_turb = max(4.36, Nu_turb);
        if Re_coolant(i) <= 2000
            Nu_l(i) = 4.36;
        elseif Re_coolant(i) >= 3000
            Nu_l(i) = Nu_turb;
        else
            gamma_trans = (Re_coolant(i) - 2000) / 1000.0;
            Nu_l(i) = (1.0 - gamma_trans) * 4.36 + gamma_trans * Nu_turb;
        end
        h_l(i) = max(100.0, (Nu_l(i) * k_coolant(i)) / hydraulic_D(i)); % liquid film coefficient [W/m^2-K] (Heister EQ 6.19)
        
        % Fins: rectangular land/rib, adiabatic tip
        m_fin = sqrt(max(0.0, h_l(i)) * P_fin(i) / (k_w_current(i) * A_c_fin(i)));
        eta_fin(i) = tanh(m_fin * h_c_x(i)) / max(1e-6, m_fin * h_c_x(i));
    ...
    T_m = 0.5 * T_m_old + 0.5 * T_m_column';
    ...
    for i = points
        ...
        if j >= 1 && j <= steps
            ...
            if coolant == "isopropyl alcohol"
                P_kPa = max(100.0, min(50000.0, real(P_coolant(i)) / 1000));
                T_eval = max(270.0, min(580.0, real(T_coolant(j))));
                rho_j_est = Data.F_rho(T_eval, P_kPa);
            else
                rho_j_est = double(py.CoolProp.CoolProp.PropsSI('D', 'T', real(T_coolant(j)), 'P', real(P_coolant(i)), coolant));
            end
            ...
            % Apply pressure drop
            P_coolant(j) = max(101325, real(P_coolant(i) - deltaP_major - deltaP_minor - deltaP_accel));
            T_coolant(j) = max(270.0, min(650.0, real(T_coolant(j))));
        end
    end
