...
function [X, U, K] = TrajectoryManager(t, constantsTOAD)
    ...
    % Indexes 
    n_low = find(Traj.Time <= t, 1, "last");    n_high = n_low + 1;
    T_low = Traj.Time(n_low);     T_high = Traj.Time(n_high);
    dt = T_high - T_low;
    
    %% Interpolation
    % Gains
        K_low = Feedback(n_lower,:);
        K_up  = Feedback(n_upper,:);
        K = K_low + (K_up - K_low) .* ((t - T_low) / dt);
    
    % States
        X_low = States(n_lower,:);
        X_up  = States(n_upper,:);
        X = X_low + (X_up - X_low) .* ((t - T_low) / dt);
    
    % Inputs
        U_low = Inputs(n_lower,:);
        U_up  = Inputs(n_upper,:);
