...
max_mdot = 2.72; % Inlet lbm/s at 100% Throttle
max_pressure = 500; % Inlet psia at 100% Throttle
throttle_array = [40, 65, 100]; % Throttle Points to Run

m_dot_array = max_mdot .* throttle_array / 100; % Inlet Mass Flowrates (lbm/s)
pressure_array = max_pressure .* throttle_array / 100; % Inlet Pressures (psia) 

for j = 1:1:size(m_dot_array, 2)
    % Inlet environment definition
    m_dot = m_dot_array(j) * u.LB2KG /2; % kg/s
    T_l = 293.16; % K
    P_l = pressure_array(j) * u.PSI2PA; % inlet pressure [PA]
    [mu_lb, ~, rho_l, ~, ~, ~] = IPA_properties(T_l, P_l);
    
    % Manifold Geometry Definition
    A_max = 0.07068583471 * 5 * u.IN2M^2;
    A_min = 0.015393804 * 5 * u.IN2M^2;  
    ...
    % Pressure loss
    roughness_table = readmatrix("/Material Data/surface_roughness.xlsx",'Range','A12:E16');
...
legend("IPA Velocity", "Pressure Drop", "Location", "west")
xlabel("Length")
title("IPA Manifold Pressure Loss (1.171 lb/s)")
